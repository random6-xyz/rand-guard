...
use edr_common::{
    ...
    NetworkBindEvent, NetworkConnectEvent, NetworkDirection, NetworkFamily, NetworkListenEvent,
...
fn try_sys_enter_network_sockaddr(ctx: TracePointContext, kind: EventKind) -> Result<u32, i64> {
    ...
    // sys_enter_connect/sys_enter_bind tracepoint layout:
    //   offset 16: fd (int)
    //   offset 24: uservaddr/umyaddr (struct sockaddr *)
    //   offset 32: addrlen (int)
...
fn try_sys_enter_listen(ctx: TracePointContext) -> Result<u32, i64> {
    ...
    // sys_enter_listen tracepoint layout:
    //   offset 16: fd (int)
    //   offset 24: backlog (int)
    let fd = unsafe { ctx.read_at::<i32>(16)? };
...
fn fill_header(
    header: &mut edr_common::EventHeader,
    kind: EventKind,
    size: u16,
    pid: u32,
    tid: u32,
